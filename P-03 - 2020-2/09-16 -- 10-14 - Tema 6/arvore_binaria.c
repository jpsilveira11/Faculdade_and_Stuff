...
int NodeHeight(TipoArvore **tree)
{
    int leftHeight,rightHeight,finalHeight;

    if(!TestaArvoreBinariaVazia(tree))
        {
            if(!TestaArvoreBinariaVazia((*tree)->esquerda))
                leftHeight=NodeHeight((*tree)->esquerda);
            else
                leftHeight=0;

            if(!TestaArvoreBinariaVazia((*tree)->direita))
                rightHeight=NodeHeight((*tree)->direita);
            else
                rightHeight=0;
            ...
            if(TestaArvoreBinariaVazia((*tree)->esquerda)&&TestaArvoreBinariaVazia((*tree)->direita))
                finalHeight++;
